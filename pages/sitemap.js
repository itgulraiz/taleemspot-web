          ...
          <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
            ...
            {searchTerm && (
              <div className="mb-4 text-gray-700">
                ...
                <span className="ml-2 text-sm text-gray-500">({filteredLinks.length} links found)</span>
              </div>
            )}

            {/* AdSense Banner */}
            <AdSenseBanner slot="sitemap-top-banner" format="horizontal" />
            ...
            <div className="space-y-8">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {(searchTerm ? subjectLinks.filter(link => 
                    link.title.toLowerCase().includes(searchTerm.toLowerCase()) || 
                    link.url.toLowerCase().includes(searchTerm.toLowerCase())
                  ) : subjectLinks.slice(0, 30)).map((link, index) => (
                    <Link 
                      href={link.url} 
                      key={index}
                      className="p-3 border border-gray-200 rounded hover:bg-green-50 hover:border-green-300 transition-colors"
                ...
                {!searchTerm && subjectLinks.length > 30 && (
                  <div className="mt-4 text-center">
                    <span className="text-gray-500">Showing 30 of {subjectLinks.length} subjects. Use search to find more.</span>
                  </div>
                )}
              </div>
              ...
              <div>
                <h2 className="text-xl font-semibold mb-4 text-gray-800 flex items-center">
                  <span className="bg-orange-100 text-orange-700 p-1 rounded mr-2">
                    <Download className="h-5 w-5" />
                  </span>
                  Resources
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {(searchTerm ? resourceLinks.filter(link => 
                    link.title.toLowerCase().includes(searchTerm.toLowerCase()) || 
                    link.url.toLowerCase().includes(searchTerm.toLowerCase())
                  ) : resourceLinks.slice(0, 30)).map((link, index) => (
                    <Link 
                      href={link.url} 
                      key={index}
                      className="p-3 border border-gray-200 rounded hover:bg-green-50 hover:border-green-300 transition-colors"
                ...
                {!searchTerm && resourceLinks.length > 30 && (
                  <div className="mt-4 text-center">
                    <span className="text-gray-500">Showing 30 of {resourceLinks.length} resources. Use search to find more.</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {/* AdSense Banner */}
            <div className="mt-8">
              <AdSenseBanner slot="sitemap-bottom-banner" format="horizontal" />
            </div>
          </div>
        </div>
