...
// Helper function to parse collection names
  ...
  useEffect(() => {
    const fetchResource = async () => {
      ...
      try {
        ...
        const resourceRef = doc(db, 'resources', documentId);
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                          By {resource.author} {resource.authorImage && (
                            <img 
                              ...
                              alt={`${resource.author}'s profile`} 
                              className="inline w-6 h-6 rounded-full ml-2"
                              onError={(e) => {
                                e.target.onerror = null;
                                e.target.parentNode.replaceWith(e.target.previousSibling);
                              }}
                            />
                          )}
                        </p>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 border border-gray-200 dark:border-gray-700">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {relatedResources.map(item => (
                      <Link 
                        key={item.id} 
                        href={`/resource/${item.class}/${item.type}/${item.documentId}`} 
                        className="block bg-white dark:bg-gray-700 p-4 rounded-lg shadow hover:shadow-md transition-shadow"
                      >
