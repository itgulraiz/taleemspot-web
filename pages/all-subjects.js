...
export async function getStaticProps() {
  try {
    ...
    // Get all unique subjects from all collections
      ...
      try {
        ...
        if (!snapshot.empty) {
          ...
          snapshot.forEach(doc => {
            ...
            let subjectEntry = subjectsMap.get(subjectName) || {
              name: subjectName,
              paperCount: 0,
              classesAvailable: new Set(),
              boardsAvailable: new Set(),
              latestYear: 0,
              paths: []
            };
            
            // Add class to this subject's available classes
            subjectEntry.classesAvailable.add(displayName);
            
            // If this document has subjects array, process it
            if (data.subjects && Array.isArray(data.subjects)) {
              // Count papers
              subjectEntry.paperCount += data.subjects.length;
              
              // Track boards and latest year
              data.subjects.forEach((paper, index) => {
                if (paper.board) {
                  subjectEntry.boardsAvailable.add(paper.board);
                }
                
                if (paper.year && paper.year > subjectEntry.latestYear) {
                  subjectEntry.latestYear = paper.year;
                }
                
                // Add path to this specific subject's papers
                subjectEntry.paths.push({
                  ...
                  year: paper.year,
                  board: paper.board,
                  path: `/${collectionName}/${subjectName}/${index}`
                });
              });
            }
            
            // Update the map with the enriched subject entry
            subjectsMap.set(subjectName, subjectEntry);
          });
        }
      } catch (error) {
    ...
    // Convert the map to an array and format for the UI
    const subjects = Array.from(subjectsMap.entries()).map(([name, data]) => ({
      name: name,
      paperCount: data.paperCount,
      classesAvailable: Array.from(data.classesAvailable),
      boardsAvailable: Array.from(data.boardsAvailable),
      latestYear: data.latestYear,
      paths: data.paths.sort((a, b) => b.year - a.year).slice(0, 5), // Get 5 most recent paths
...
const AllSubjects = ({ subjects, totalSubjects }) => {
  ...
  useEffect(() => {
    if (searchTerm.trim() === '') {
      ...
      const filtered = subjects.filter(subject => 
        ...
        subject.classesAvailable.some(cls => cls.toLowerCase().includes(searchTerm.toLowerCase())) ||
        subject.boardsAvailable.some(board => board.toLowerCase().includes(searchTerm.toLowerCase()))
      );
  ...
  return (
    <>
      <Head>
        <title>All Subjects - TaleemSpot</title>
        <meta name="description" content={`Browse all ${totalSubjects}+ subjects for 9th, 10th, 11th, 12th classes, ECAT and MDCAT including Physics, Chemistry, Biology, Math and more.`} />
        <meta name="keywords" content="subjects, Physics, Chemistry, Biology, Math, English, Urdu, Pakistan education, past papers" />
        <link rel="canonical" href="https://taleemspot.com/all-subjects" />
      </Head>
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
            <div className="mb-6">
              ...
              <p className="text-lg text-gray-600">Browse {totalSubjects} subjects across different classes and boards</p>
            </div>

            {/* AdSense Banner */}
            <AdSenseBanner slot="all-subjects-top-banner" format="horizontal" />

            {/* Search Results Label */}
            {searchTerm && (
              <div className="mb-4 text-gray-700">
                Showing results for: <span className="font-medium text-green-600">"{searchTerm}"</span>
                <span className="ml-2 text-sm text-gray-500">({filteredSubjects.length} subjects found)</span>
              </div>
            )}
                    ...
                    <div className="p-5">
                      <div className="flex items-center mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{subject.paperCount} past papers</p>
                        </div>
                      </div>
                      
                      <div className="mb-4">
                        <div className="text-sm text-gray-600 mb-2">Available in:</div>
                        <div className="flex flex-wrap gap-2">
                          {subject.classesAvailable.slice(0, 3).map((cls, i) => (
                            <span key={i} className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">
                              {cls}
                            </span>
                          ))}
                          {subject.classesAvailable.length > 3 && (
                            <span className="bg-gray-100 text-gray-700 text-xs px-2 py-1 rounded">
                              +{subject.classesAvailable.length - 3} more
                            </span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="mb-4">
                        <div className="text-sm text-gray-600 mb-2">Latest papers:</div>
                        <div className="space-y-2">
                          {subject.paths.slice(0, 2).map((path, i) => (
                            <Link
                              key={i}
                              href={path.path}
                              className="block bg-gray-50 hover:bg-gray-100 rounded p-2 text-sm transition-colors"
                            >
                              <div className="flex justify-between">
                                <span>{path.displayClass} {path.year}</span>
                                <span className="text-green-600">{path.board}</span>
                              </div>
                            </Link>
                          ))}
                        </div>
            ...
            {/* AdSense Banner */}
            <div className="mt-8">
              <AdSenseBanner slot="all-subjects-bottom-banner" format="horizontal" />
            </div>
          </div>
        </div>
