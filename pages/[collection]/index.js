...
export async function getStaticPaths() {
  return {
    paths: [
      { params: { collection: 'Punjab9thPastPapers' } },
      { params: { collection: 'Punjab10thPastPapers' } },
      { params: { collection: 'Punjab11thPastPapers' } },
      { params: { collection: 'Punjab12thPastPapers' } },
      { params: { collection: 'PunjabECATPastPapers' } },
      { params: { collection: 'PunjabMDCATPastPapers' } },
    ],
    fallback: 'blocking'
  };
}
  ...
  try {
    ...
    snapshot.forEach(doc => {
      ...
      subjects.push({
        ...
        items: data.subjects?.map((subject, index) => {
...
const CollectionPage = ({ collectionName, displayName, subjects, collections }) => {
  ...
  return (
    <>
      <Head>
        <title>{displayName} Past Papers - TaleemSpot</title>
        <meta name="description" content={`Download ${displayName} past papers from all Punjab boards. Get access to Biology, Physics, Chemistry, Mathematics and more subjects.`} />
        <meta name="keywords" content={`past papers, ${displayName}, Punjab board, education, Pakistan`} />
        <meta property="og:title" content={`${displayName} Past Papers - TaleemSpot`} />
        <meta property="og:description" content={`Download ${displayName} past papers from all Punjab boards.`} />
        <meta property="og:url" content={`https://taleemspot.com/${collectionName}`} />
        <meta property="og:type" content="website" />
        <link rel="canonical" href={`https://taleemspot.com/${collectionName}`} />
      </Head>
            ...
            <div className="flex justify-between items-center py-3">
              ...
              <div className="hidden md:flex flex-1 max-w-2xl mx-8">
                <div className="relative w-full">
                  ...
                  <input
                    type="text"
                    placeholder={`Search ${displayName} Subjects...`}
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
            <div className="text-center mb-6">
              <h1 className="text-3xl font-bold text-gray-800 mb-2">{displayName} Past Papers</h1>
              <p className="text-lg text-gray-600">Find and download past papers for all subjects</p>
            </div>

            {/* AdSense Banner */}
            <AdSenseBanner slot="collection-top-banner" format="horizontal" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-6">
              {filteredSubjects.length > 0 ? (
                filteredSubjects.map((subject) => (
                  <Link 
                    key={subject.id}
                    href={`/${collectionName}/${subject.id}`}
                    className="bg-white rounded-lg border border-gray-200 hover:shadow-lg transition-all duration-300 p-4 flex flex-col"
                  >
                ...
                <div className="col-span-full text-center py-8">
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <h3 className="text-lg font-medium text-gray-600">No subjects found</h3>
            ...
            {/* AdSense Banner */}
            <div className="mt-8">
              <AdSenseBanner slot="collection-bottom-banner" format="horizontal" />
            </div>
          </div>
        </div>
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm">
                  ...
                  {collections.slice(0, 5).map(coll => (
                    <li key={coll.id}>
                      <Link href={`/${coll.id}`} className="text-gray-400 hover:text-white flex items-center">
                        <ChevronDown className="h-3 w-3 mr-2 transform rotate-90" /> {coll.name} Past Papers
                      </Link>
                    </li>
                  ))}
