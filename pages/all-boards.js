...
export async function getStaticProps() {
  try {
    ...
    // Extract unique boards and count resources
    const boardsMap = {};
    allResources.forEach(resource => {
      ...
      if (!boardsMap[board]) {
        boardsMap[board] = {
          name: board,
          count: 1,
          classes: new Set([resource.class])
        };
      } else {
        boardsMap[board].count += 1;
        boardsMap[board].classes.add(resource.class);
      }
    });
    
    // Convert to array and sort alphabetically
    const boards = Object.values(boardsMap).map(board => ({
      name: board.name,
      count: board.count,
      classes: Array.from(board.classes),
...
const AllBoards = ({ boards, totalResources }) => {
  ...
  useEffect(() => {
    if (searchTerm.trim() === '') {
      ...
      const filtered = boards.filter(board => 
        ...
        board.classes.some(cls => cls.toLowerCase().includes(searchTerm.toLowerCase()))
      );
  ...
  return (
    <>
      <Head>
        <title>All Boards - TaleemSpot</title>
        <meta name="description" content="Browse all education boards including Lahore, Faisalabad, Multan, Rawalpindi and more on TaleemSpot." />
        <meta name="keywords" content="education boards, Punjab boards, Pakistan, Lahore Board, Faisalabad Board, Multan Board" />
        <link rel="canonical" href="https://taleemspot.com/all-boards" />
      </Head>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
            <div className="mb-6">
              <h1 className="text-3xl font-bold text-gray-800 mb-2">All Education Boards</h1>
              <p className="text-lg text-gray-600">Browse {boards.length} education boards with {totalResources}+ past papers</p>
            </div>

            {/* AdSense Banner */}
            <AdSenseBanner slot="all-boards-top-banner" format="horizontal" />

            {/* Search Results Label */}
            {searchTerm && (
              <div className="mb-4 text-gray-700">
                Showing results for: <span className="font-medium text-green-600">"{searchTerm}"</span>
                <span className="ml-2 text-sm text-gray-500">({filteredBoards.length} boards found)</span>
              </div>
            )}
              ...
              {filteredBoards.length > 0 ? (
                      ...
                      <div className="flex items-center space-x-3 mb-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-600 mt-1">
                            {board.count} past papers
                          </p>
                        </div>
                      </div>
                      <div className="mt-3">
                        <div className="flex flex-wrap gap-1">
                          {board.classes.slice(0, 3).map((cls, idx) => (
                            <span key={idx} className="inline-block bg-blue-50 text-blue-600 text-xs px-2 py-1 rounded">
                              {cls} {!cls.includes('ECAT') && !cls.includes('MDCAT') ? 'Class' : ''}
                            </span>
                          ))}
                          {board.classes.length > 3 && (
                            <span className="inline-block bg-gray-50 text-gray-600 text-xs px-2 py-1 rounded">
                              +{board.classes.length - 3} more
                            </span>
                          )}
                        </div>
                ...
                <div className="col-span-full text-center py-12">
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <h3 className="text-xl font-medium text-gray-600">No boards found</h3>
            ...
            {/* AdSense Banner */}
            <div className="mt-8">
              <AdSenseBanner slot="all-boards-bottom-banner" format="horizontal" />
            </div>
          </div>
        </div>
