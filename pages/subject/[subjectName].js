...
export async function getStaticPaths() {
  try {
    ...
    // Get all subjects from all collections
    const subjectSet = new Set();
    
    // Process each collection to extract unique subjects
    ...
    // Create paths for the top 50 most common subjects to pre-render
    // The rest will be generated on-demand
    const paths = Array.from(subjectSet).slice(0, 50).map(subject => ({
      params: { subjectName: subject.toLowerCase().replace(/\s+/g, '-') }
    }));
...
export async function getStaticProps({ params }) {
  try {
    ...
    // Convert hyphenated URL slug back to potential subject name
    const possibleSubjectNames = [
      subjectName.replace(/-/g, ' '),  // Convert dashes to spaces
      subjectName.replace(/-/g, ' ').toLowerCase(),  // Lowercase version
      subjectName.replace(/-/g, ' ').toUpperCase(),  // Uppercase version
      // Convert first letter of each word to uppercase
      subjectName.replace(/-/g, ' ').replace(/\w\S*/g, w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase())
    ];

    // Get all resources for this subject
    let subjectResources = [];
    let actualSubjectName = '';
    
    // Process each collection
    outerLoop: for (const collectionName of collections) {
      try {
        const collRef = collection(db, collectionName);
        const snapshot = await getDocs(collRef);
        ...
        if (!snapshot.empty) {
          ...
          for (const doc of snapshot.docs) {
            // Check if this document matches any of the possible subject names
            if (possibleSubjectNames.includes(doc.id) || 
                possibleSubjectNames.includes(doc.id.toLowerCase()) ||
                possibleSubjectNames.some(name => doc.id.toLowerCase().includes(name))) {
              
              actualSubjectName = doc.id; // Save the actual name from Firestore
              const data = doc.data();
              
              // Process each paper for this subject
              if (data.subjects && Array.isArray(data.subjects)) {
                data.subjects.forEach((subject, index) => {
                  if (subject.url && subject.board && subject.year) {
                    // Create a record for each paper
              ...
              // If we found a match, we can continue to the next collection
              break;
            }
          }
        }
    ...
    subjectResources.sort((a, b) => b.year - a.year);
    ...
    // Extract classes for this subject
    const subjectClasses = [...new Set(subjectResources.map(resource => resource.displayClass))];

    // Extract boards for this subject
    const subjectBoards = [...new Set(subjectResources.map(resource => resource.board))];

    // Extract years for this subject
    const subjectYears = [...new Set(subjectResources.map(resource => resource.year))].sort((a, b) => b - a);
...
const SubjectPage = ({ subjectName, resources, classes, boards, years, totalResources }) => {
  ...
  useEffect(() => {
    ...
    if (selectedClass !== 'all') {
      filtered = filtered.filter(item => item.displayClass === selectedClass);
    }
    
    if (selectedBoard !== 'all') {
      filtered = filtered.filter(item => item.board === selectedBoard);
    }
    ...
    if (searchTerm.trim() !== '') {
      ...
      filtered = filtered.filter(item => {
        return searchTerms.every(term => 
          ...
          item.displayClass.toLowerCase().includes(term)
        );
      });
    }
    ...
    setCurrentPage(1);
  }, [searchTerm, selectedClass, selectedBoard, selectedYear, resources]);
          ...
          <div className="flex items-center space-x-3 mb-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600 mt-1">
                {resource.displayClass} • {resource.board} Board
              </p>
            </div>
          </div>
  ...
  return (
    <>
      <Head>
        <title>{subjectName} Past Papers - TaleemSpot</title>
        <meta name="description" content={`Download ${subjectName} past papers for all classes and boards. Access ${totalResources}+ past papers for ${subjectName} subject.`} />
        <meta name="keywords" content={`${subjectName}, past papers, education, Pakistan, ${classes.join(', ')}, Punjab board`} />
        <link rel="canonical" href={`https://taleemspot.com/subject/${subjectName.toLowerCase().replace(/\s+/g, '-')}`} />
      </Head>
      ...
      <div className="min-h-screen bg-gray-50">
            ...
            <div className="hidden md:flex justify-between items-center py-3">
              ...
              <div className="flex flex-1 max-w-2xl mx-8">
                <div className="relative w-full">
                  ...
                  <input
                    type="text"
                    placeholder={`Search ${subjectName} past papers...`}
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
            <div className="mb-6">
              <h1 className="text-3xl font-bold text-gray-800 mb-2">{subjectName} Past Papers</h1>
              <p className="text-lg text-gray-600">Download {totalResources} past papers for {subjectName} subject</p>
            </div>

            {/* AdSense Banner */}
            <AdSenseBanner slot="subject-top-banner" format="horizontal" />
            ...
            <div className="mb-4 text-gray-700">
              {(searchTerm || selectedClass !== 'all' || selectedBoard !== 'all' || selectedYear !== 'all') && (
                <div>
                  <span>Filtered results: </span>
                  {selectedClass !== 'all' && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800 mr-2">Class: {selectedClass}</span>}
                  {selectedBoard !== 'all' && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 mr-2">Board: {selectedBoard}</span>}
                  {selectedYear !== 'all' && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 mr-2">Year: {selectedYear}</span>}
                    ...
                    if (pageNum > 0 && pageNum <= totalPages) {
                      return (
                        <button 
                          ...
                          className={`py-2 px-4 border ${currentPage === pageNum ? 'bg-purple-600 text-white border-purple-600' : 'bg-white text-gray-700 hover:bg-gray-50'} text-sm font-medium`}
                        >
                          {pageNum}
                        </button>
                      );
            ...
            {/* AdSense Banner */}
            <div className="mt-8">
              <AdSenseBanner slot="subject-bottom-banner" format="horizontal" />
            </div>
          </div>
        </div>
